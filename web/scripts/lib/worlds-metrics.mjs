 ...
 * - **`ef2ad991f81dfddf` — `scored` 28 of 45, `byShares` 30 of 45.** The DEC-885 refresh (2026-09-21):
 *   one full `baseline` tour on the new hash (`--only baseline --no-captures`), 28 worlds scoring
 *   W3 and `qualifyingPlanes` 30, with `shenmeng` and `zhalfir` again the two that qualify on their
 *   cards and present one band. Same roster, same card-to-plane assignment (report 9.2.3: 0 moved),
 *   so the unchanged pair is expected — but it was read off the tour, not copied from the row above.
 *
...
export const W3_DOMAIN_SIZE = Object.freeze({
  ...
  ef2ad991f81dfddf: Object.freeze({ scored: 28, byShares: 30 }),
});
 ...
 *
 * Swept through this function on a **named fixture, because the per-row figures move with it**
 * (DEC-845, rider 3 of the DEC-844 review). `evictions` is an integer counter, so `rate` and
 * `halfRate` each carry up to one count of quantisation and the per-row drift inherits it; two
 * honest sweeps at different `r0` or span disagree by a few tenths of a point on every row while
 * agreeing on the conclusion. Naming the fixture is what makes the table re-runnable:
 *
 * > **the fixture** — a saturated 1,024-layer timeline (`resident === layers` at every sample, so
 * > there is no fill to exclude and the tail is the whole window), instantaneous rate declining
 * > linearly from **r0 = 18 evictions/s** to `r0(1−d)` over a span of **45 s** (the gate's
 * > `W4_EVICTION_MAX_S`, its longest window and so its least-quantised one) sampled every
 * > **250 ms** (the gate's `W4_EVICTION_SAMPLE_MS`), with the cumulative counter floored to an
 * > integer: `evictions(t) = floor(r0·(t − d·t²/2S))`, 181 samples.
 *
 * | end-to-end decline | `drift` on the fixture | closed form | `converged` |
 * |---|---|---|---|
 * | 5.4% (DEC-835's figure) | **1.27%** | 1.39% | **true** |
 * | 10% | 2.47% | 2.63% | true |
 * | 20% | 5.35% | 5.56% | true |
 * | 30% | 8.72% | 8.82% | true |
 * | 35% | 10.48% | 10.61% | false |
 *
 * The closed-form column is the same fixture with the `floor` taken off — `drift = (d/4)/(1−d/2)`,
 * which it reproduces to three decimals — so the gap between the columns *is* the integer counter
 * and nothing else. Run it at a smaller `r0` or a shorter span and the measured column moves by a
 * few tenths in either direction; the conclusion does not.
 *
 * The verdict flips at an end-to-end decline of **33.8% on this fixture and 33.3% unquantised**, so
 * **10% here is roughly a 33% tolerance on the trend**. That is still the right number for what this
 * rule is for — refusing a *fill* read as a steady state, which is a factor-of-several error, not a
 * few percent — but it must not be quoted as catching a few-percent settle. It does not.
 *
