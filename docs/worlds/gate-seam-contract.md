...
| `?layers=N`             | R1    | Pins the pool size alone, reported back **after** the `max(0, min(N, MAX_ARRAY_TEXTURE_LAYERS − 32))` clamp. **It is not `?quality=N`** — that one already exists (`pinnedQualityTier` in `adaptiveQuality.ts`) and moves four quantities at once (`pixelRatioCap`, `bloomScale`, `bloomLevels`, `glow`). Routed through the tier, the expected-GREEN `?layers=128` row would also be measuring dpr, bloom and glow.   |
