...
**A reader must mask.** Byte 7 now ranges over 0-253, so a consumer that reads it whole gets a hue class of 132 for a mono-green card and indexes `uHues` — a seven-element array — far past its end. Both decoders take the low three bits (`decode.ts`'s `hueClass` accessor, and `int(mod(aClass.y + 0.5, 8.0))` in the star vertex shader; `mod` rather than a bitwise `and` because that shader compiles as GLSL ES 1.00). This is why the change bumps `contractVersion`: the failure is silent, not loud.
...
2. **Nothing reads byte 7 unmasked.** Two consumers do: `decode.ts`'s `hueClass` accessor (`& 0b111`) and the star vertex shader (`int(mod(aClass.y + 0.5, 8.0))` — `mod` rather than a bitwise `and`, because that shader compiles as GLSL ES 1.00). `filters/evaluate.ts` is unchanged and goes through the masked accessor, so **Phase 4 filter behaviour is byte-identical**; exact colour filtering is a separate leg.
