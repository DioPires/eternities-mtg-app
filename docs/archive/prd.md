...
**Per card:** `oracle_id`, name, mana cost, type line, oracle text (per face), colour identity, layout, plane slug, first-printing id, first-printing release date, first-printing rarity (with `special` → rare and `bonus` → mythic), card types for filtering (front face), number of included printings, layout position (section 8), and the printings list.
...
4. Blind Eternities cards are scattered through the supercluster volume, avoiding plane interiors, with density highest between neighbouring planes. This dust is the Blind Eternities; it is also a selectable target (see 5.7). Focusing it brightens the dust, fades plane labels, and tethers the camera to the multiverse centre with plane-level distance limits; the card sheet tier (5.5) applies to dust exactly as to stars.
...
**Rotation**
13. Stars orbit the galactic centre. Implementation is a rigid rotation of the plane's transform plus a bounded differential applied in the vertex shader (inner stars faster). The inner/outer angular speed ratio must not exceed 1.3, so arms remain readable over a 30-minute session.
...
6. While a card is focused, its plane's rotation eases to a stop over 1 s and eases back over 1 s when focus is released. Other planes keep moving.
...
8. Up to 24 printings occupy one ring; 25–72 use three concentric rings; beyond 72, the outermost ring is capped and the remainder is listed in the card panel. Cards with one printing show no planets.
...
1. The camera is always tethered to a focus: multiverse centre, a plane centre, the Blind Eternities, or a card. Orbit and zoom operate around the focus, within per-level distance limits.
...
3. Focusing a card from outside its plane is a two-stage fly-to: fly to and frame the plane, hold 0.4 s, then fly to the card. Combined duration is capped at 3.5 s. Any input cancels the remainder (5.7.3).
...
2. Filter chips, beneath the breadcrumb: one chip per active facet value, each individually removable, with a clear-all control and a live count of matching cards.
...
4. Selecting a plane flies to it. Selecting a card performs the two-stage fly-to (6.2.3). Selecting a set flies to the set's plane and adds a set filter chip.
5. The search index is client-side and ships with the initial payload; no network round trip per keystroke.
...
5. Filters apply at every level and render per 5.8. Changing a filter never moves the camera.
...
3. Loading a deep link plays the intro (6.8.2) into the target rather than into the multiverse home position, applying filters before the first frame so nothing pops.
...
| GPU memory for thumbnails and card images | ≤ 96 MB | 160 MB |
...
1. The only network origins are the site itself and Scryfall's image CDN; expressed as a Content Security Policy.
...
2. Per-plane parameters live in a float `DataTexture` (one row per plane): world position, home position, radius, tilt quaternion, drift phase and amplitude, current accumulated spin angle, differential factor, and palette. A data texture avoids uniform-array limits and updates in one call per frame.
3. **Motion is computed in the vertex shader.** For each star: apply differential spin about the plane's axis using the plane's accumulated angle and the star's radius, apply the tilt, add the drift offset, translate to the plane's position, then apply the multiverse rotation. The CPU updates only the per-plane accumulated angles (so a focused plane can ease to a stop without a discontinuity) and the global angle; ~80 floats per frame.
...
8. Thumbnails: one `InstancedMesh` of quads with a fixed capacity (default 512) backed by a texture atlas (default 4096², 128 × 178 cells). An LRU loader fetches Scryfall `small` images nearest-first, decodes with `createImageBitmap`, and evicts least-recently-visible. Instances cross-fade with the star per 5.5.
9. Focused card: a rounded-box mesh with `large` front, Scryfall back, dark edge; pointer-driven tilt via a damped spring.
10. Planets: individual small sphere meshes (≤ 72), each with an `art_crop` texture, orbiting per 5.6.7.
...
5. `search.json` in the background after the first frame.
...
6. **Blind Eternities target.** The 15% working target in 9.2.2 predates the decision to include the D&D sets there. Reset it after the first pipeline report.
...
11. **Visual tunables.** Rarity size ratio, brightness percentile cap, twinkle amplitude, bloom threshold, spin periods. All have defaults in section 5; the visual review (9.3) decides where they land.
...
- `p3k` Portal Three Kingdoms · 1999-05 · `blind-eternities` (alternate-history Earth, retconned as not a plane)
...
- `gn1` Game Night · 2018-11 · `blind-eternities`
