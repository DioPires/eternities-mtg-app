...
7. Printings appear as planets orbiting the card, ordered clockwise by release date starting at 12 o'clock, evenly spaced, each textured with that printing's art crop. Planets orbit at their own rate (default: one revolution per 60 s), independent of the plane's spin, so they keep moving while the plane is paused.
   ...
   *Amended 2026-09-21 (DEC-913).* The release date ordered by is the **printing's own** `released_at` under 4.5.1's key — date, then set-type priority, then set code, collector number and id — so the planet at 12 o'clock is the card's first printing (4.5) by construction, and §2.3's cell draws and credits that same printing. The rule had been read as the printing's *set* date, which disagrees wherever a set's date does not describe the printing: on `f2be4a22ce639774` that put the wrong printing first for 458 cards, 451 of them from The List (`plst`, one set date of 2020-09-26 over six years of printings).
